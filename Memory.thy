...
lemma ssa_set_\<alpha>_eq[simp]: "ssa_set_\<alpha> (vs,s,h) = ssa_set_\<alpha> (vs,s',h')"
  apply (rule ext)
  subgoal for x
...
lemma wp_set_memory_intro[THEN consequence, wp_intro]:
  assumes "memory_\<alpha> s a \<noteq> None"
  shows "wp (set_memory s a v) (\<lambda>s'. memory_\<alpha> s' = (memory_\<alpha> s)(a := Some (Some v)) \<and> ssa_\<alpha> s = ssa_\<alpha> s' \<and> ssa_set_\<alpha> s = ssa_set_\<alpha> s')"
  using assms 
...
lemma wp_free_memory_intro[THEN consequence, wp_intro]:
  assumes "memory_\<alpha> s a \<noteq> None"
  shows "wp (free_memory s a) (\<lambda>s'. memory_\<alpha> s' = (memory_\<alpha> s)(a := None) \<and> ssa_\<alpha> s = ssa_\<alpha> s' \<and> ssa_set_\<alpha> s = ssa_set_\<alpha> s')"
  using assms
  apply (cases a; cases s; simp)
...
lemma wp_allocate_heap_intro[THEN consequence, wp_intro]:
  "wp (return (allocate_heap s)) (\<lambda>(s', a). (\<exists>a'. a = haddr a') \<and> (memory_\<alpha> s') = (memory_\<alpha> s)(a := Some None) \<and> ssa_\<alpha> s = ssa_\<alpha> s' \<and> ssa_set_\<alpha> s = ssa_set_\<alpha> s')"
...
lemma wp_allocate_stack_intro[THEN consequence, wp_intro]:
  "wp (return (allocate_stack s)) (\<lambda>(s', a). (\<exists>a'. a = saddr a') \<and> (memory_\<alpha> s') = (memory_\<alpha> s)(a := Some None) \<and> ssa_\<alpha> s = ssa_\<alpha> s' \<and> ssa_set_\<alpha> s = ssa_set_\<alpha> s')"
