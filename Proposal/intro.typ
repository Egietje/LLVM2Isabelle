...
With that, the goal of this thesis will be to create a deductive verification tool for partial correctness of LLVM-IR programs within Isabelle/HOL.
This can be achieved through the following steps, producing a minimum viable product:
...
+ use these tools on some program to show its efficacy.

Once that is done and an MVP has been produced it can be extended in multiple ways, such as:
- extending the defined subset of LLVM-IR to support more of its features, instructions, and intrinsics,
- creating another tool to port LLVM-IR output into the verifier and vice versa, preferably using Isabelle to prove it can losslessly port LLVM-IR programs,
- improving the backend using advanced verification principles to make verification easier,
- extend the verifier to prove total correctness rather than only partial correctness.
